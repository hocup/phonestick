...
function reportOrientation(event) {
  const absMin = 0;
  const absMax = 65535;
  ...
  msg = axes.reduce(
      (acc, axis) => {
        let slope = (absMax - absMin)/(scaling[axis][1] - scaling[axis][0]);
        let intercept = (absMax - absMin)/2;
        acc[axis] = msg[axis]*slope + intercept;

        acc[axis] = Math.max(acc[axis], absMin);
        acc[axis] = Math.min(acc[axis], absMax);
